...
SHAQ_IMAGE = pygame.image.load(os.path.join('Pictures', 'shaqmagic.jpg'))
SHAQ_IMAGE = pygame.transform.rotate(pygame.transform.scale(
    SHAQ_IMAGE, (BASKETBALL_WIDTH, BASKETBALL_HEIGHT)), 360)

CHARLESBARKLEY_IMAGE = pygame.image.load(os.path.join('Pictures', 'barkleysuns.jpg'))
CHARLESBARKLEY_IMAGE = pygame.transform.rotate(pygame.transform.scale(
    CHARLESBARKLEY_IMAGE, (BASKETBALL_WIDTH, BASKETBALL_HEIGHT)), 360)
...
def main():
    shaq = pygame.Rect(700, 300, BASKETBALL_WIDTH, BASKETBALL_HEIGHT)
    barkley = pygame.Rect(100, 300, BASKETBALL_WIDTH, BASKETBALL_HEIGHT)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(shaq_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(
                        barkley.x + barkley.width, barkley.y + barkley.height//2 - 2, 10, 5)
                    shaq_bullets.append(bullet)
                    #BULLET_FIRE_SOUND.play()

                if event.key == pygame.K_RCTRL and len(barkley_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(
